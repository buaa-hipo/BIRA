...
typedef struct _per_thread_t {
    ...
    int32_t threadId;
    record_t* rec;
    record_params_t* rec_p;
    std::vector<record_t*> *rec_list;
} per_thread_t;
...
event_exit(void);
static void
wrap_pre_trace_params(void *wrapcxt, OUT void **user_data);
static void
wrap_post_trace_params(void *wrapcxt, void *user_data);
/*
static void
...
static droption_t<std::string> op_arg_num
(DROPTION_SCOPE_CLIENT, "n", "0", "Target function arg num",
 "Target function arg num.");

static int tls_idx, arg_num;
...
ClientThreadStart(void *drcontext)
{
    ...
    pt->rec_list = new std::vector<record_t*>();
...
module_load_event(void *drcontext, const module_data_t *mod, bool loaded)
{
	...
	arg_num = std::stoi(op_arg_num.get_value());
        app_pc towrap = mod->start + modoffs;
#ifdef SHOW_RESULTS
        bool ok =
#endif
            drwrap_wrap(towrap, wrap_pre_trace_params, wrap_post_trace_params);
...
wrap_pre_trace_params(void *wrapcxt, OUT void **user_data)
{
    ...
    pt->rec = (record_t*)allocate(pt, sizeof(record_t));
    pt->rec_list->push_back(pt->rec);
    pt->rec[0].type = BIRA_DEFAULT;

    for (int i = 0; i < arg_num; i++) {
        pt->rec_p = (record_params_t*)allocate(pt, sizeof(record_params_t));
        pt->rec_p[0].params = (uintptr_t)drwrap_get_arg(wrapcxt, i);;
        pt->rec_p[0].type = BIRA_COLLECTED_PARAMS;
    }
    pt->rec[0].timestamps.enter = get_tsc_raw();
    pt->depth++;
}
...
wrap_post_trace_params(void *wrapcxt, void *user_data)
{
    ...
    pt->rec[0].timestamps.exit = get_tsc_raw();
